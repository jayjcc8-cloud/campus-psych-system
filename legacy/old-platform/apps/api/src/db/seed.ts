...
const teacherIdentityBindings = [
  {
    id: "teacher-identity-001",
    userId: "counselor-user-001",
  ...
  {
    id: "teacher-identity-002",
    userId: "counselor-user-002",
    counselorId: "counselor-002",
    workId: "T002",
    phoneNumber: "13900002027",
    phoneMask: "139****2027",
    openid: "teacher-openid-002",
    status: "active"
  }
] as const;
...
export function seedDatabase() {
  ...
  const insertTeacherIdentityBinding = database.prepare(`
    INSERT OR IGNORE INTO teacher_identity_bindings (
      id, user_id, counselor_id, work_id, phone_number, phone_mask, openid, status, created_at, updated_at
    ) VALUES (
      @id, @userId, @counselorId, @workId, @phoneNumber, @phoneMask, @openid, @status, @createdAt, @updatedAt
    )
  `);
  ...
  runInTransaction(() => {
    ...
    teacherIdentityBindings.forEach((binding) => {
      insertTeacherIdentityBinding.run({
        ...binding,
        createdAt: timestamp,
        updatedAt: timestamp
      });
