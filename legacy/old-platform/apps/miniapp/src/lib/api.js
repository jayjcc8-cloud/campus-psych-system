...
import { saveStudentRoleSession, saveTeacherRoleSession } from "./role-mode";
...
export async function phoneOneClickLogin(code, options = {}) {
  const response = await Taro.request({
  ...
  if (response.data?.role === "teacher" || response.data?.role === "counselor") {
    return saveTeacherRoleSession(response.data);
  }
...
export async function loginTeacher(payload) {
  const response = await Taro.request({
    url: `${API_BASE_URL}/auth/teacher/login`,
    method: "POST",
    data: payload,
    header: {
      "Content-Type": "application/json"
  ...
  if (response.statusCode < 200 || response.statusCode >= 300) {
    ...
    throw new MiniappApiError(responsePayload?.message ?? "Teacher login failed.");
  }
