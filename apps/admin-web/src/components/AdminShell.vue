...
function logout() {
  localStorage.removeItem("teacher_support_admin_token");
...
<template>
  <main class="admin-layout">
    <aside class="nav-panel">
      ...
      <h1>匿名支持后台</h1>
      <p class="muted">以尊重和克制的方式承接教师支持请求。</p>
      ...
      <button v-if="getToken()" class="secondary full" @click="logout">退出登录</button>
    </aside>
